export const navigation = [
    {
        id: "0",
        title: "Home",
        url: "/",
    },
    {
        id: "1",
        title: "About",
        url: "/about",
    },
    {
        id: "2",
        title: "Projects",
        url: "/projects",
    },
    {
        id: "3",
        title: "Contact",
        url: "/contact",
    },
];

export const projects = {
    web: [
        {
            name: 'Coffee N Comics Online Ordering Apps',
            description: 'Mobile ordering app, Companion app and boss(manager/inventory) app for Coffee N Comics.',
            tech: ['React', 'Supabase', 'Next.js', 'Typescript', 'Vercel', 'Git', 'GitHub'],
            link: null,
            liveDemo: 'https://coffeencomicslabs.com',
            images: [
                'projects/cnc.png',
                'projects/mobile-profile.jpeg',
                'projects/labs.png',
            ]
        },
        {
            name: 'Game Recommendation System',
            description: 'Developed a proof-of-concept game recommendation engine that allows users to search for games by features,' +
                ' vendors, themes, or similar titles, generating tailored recommendations to enhance user engagement.',
            tech: ['Typescript', 'Google Apps Script', 'Git', 'GitHub', 'Vercel', 'Next.js'],
            link: '',
            liveDemo: '',
            images: []
        },
        {
            name: 'My Portfolio Website',
            description: 'Why Hello There :)',
            tech: ['React', 'React Router', 'DNS', 'Vite', 'Vercel', 'Git', 'GitHub', 'TailwindCSS'],
            link: 'https://github.com/nowyourejinxed/Portfolio',
            liveDemo: '',
            images: [
                '/sabrina-jackson.svg',
                '/projects/kenobi.gif'
            ]
        },
        {
            name: 'UNR ACM Chapter Website',
            description: 'Redesigned the website for the UNR ACM chapter.',
            tech: ['HTML5', 'CSS', 'Apache', 'SSH', 'SCP'],
            link: '',
            liveDemo: 'https://acm.cse.unr.edu/index.html',
            images: [
                'projects/home.jpeg',
                'projects/acm-about.jpeg',
                'projects/about.jpeg',
            ]
        },
    ],
    games: [
        {
            name: 'Psychic Riches',
            description: 'A "pick" game where you have to guess the correct symbol on the face of the card. You win by ' +
                'selecting the correct symbol and each correct match has the ability to trigger a bonus. This game was originally' +
                'made to demonstrate sdk integration with the Horseplay platform, but is in queue to be published on the site for players!',
            tech: ['Javascript', 'Phaser.js', 'Vite', 'Vercel', 'Git', 'Bitbucket', 'Blender', 'TexturePacker'],
            link: '',
            liveDemo: '',
            images: [
            ]
        },
        {
            name: 'Happy Bee',
            description: 'Bee themed endless runner.',
            tech: ['Unity', 'C#'],
            link: '',
            liveDemo: '',
            images: [

            ]
        },
        {
            name: 'C4 Racer',
            description: 'A crash proto game.',
            tech: ['Typescript', 'Phaser.js', 'Vite', 'Vercel', 'Git', 'Bitbucket', 'Blender', 'TexturePacker'],
            link: '',
            liveDemo: '',
            images: [
            ]
        },
        {
            name: 'Poroific',
            description: 'A dynamic survival game featuring Poro, who has to survive the onslaught of Fishbones!' +
                ' Made within 38 hours for the 2024 Biggest Little Hackathon using Unity.',
            tech: ['Unity', 'C#', 'Git', 'GitHub'],
            link: 'https://github.com/nowyourejinxed/Poroific',
            liveDemo: '',
            images: [
                'projects/poro-main.jpeg',
                'projects/poro-ing.jpeg',
                'projects/poro-go.jpeg',
                'projects/logo-poro.jpeg',
            ]
        },
        {
            name: 'SideQuest',
            description: 'Daily RPG mobile game built for iOS and Android : "Wrestling with a dark past and seeking solitude, you decide to take a job as a lighthouse attendant in Painter’s Cove,' +
                ' a sleepy coastal town. However, not long after arrival, you discover the lighthouse and surrounding property are haunted. Complete daily quests, befriend ghosts, explore labyrinthine island caves, and uncover the mysteries surrounding your new home on this spooky nautical adventure."',
            tech: ['Unity', 'C#', 'Git', 'LFS', 'GitHub', 'X-Code'],
            link: '',
            liveDemo: 'https://umbranatigames.com',
            images: [
                'projects/side-quest.png',
                'projects/sq-game.jpeg',
            ]
        },
    ]

}

export const myPhotos = [
    {
        description: '',
        url: "/me/me.png"
    },
    {
        description: '',
        url: "/me/italy.png"
    },
    {
        description: '',
        url: "/me/japan.png"
    },
    {
        description: '',
        url: "/me/ski.png"
    },
    {
        description: '',
        url: "/me/potatoes.png"
    },
    {
        description: '',
        url: "/me/catwoman.png"
    },
    {
        description: '',
        url: "/me/comics.png"
    },
    {
        description: 'Fun Fact! I love penguins!',
        url: "/me/pengu.png"
    },
    {
        description: '',
        url: "/me/jinx.png"
    },
    {
        description: '',
        url: "/me/snow.png"
    },
    {
        description: 'My true love. The river duck! <3',
        url: "/me/duck.png"
    },
]

